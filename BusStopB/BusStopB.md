...
uint8_t broadcastAddress[] = {0xEC, 0x64, 0xC9,
                              0x86, 0x13, 0xC0};
...
int rgScheduled[nStopNum][nStopNum] = {
    {100, 50}, {50, 100}};  // store in seconds
...
void IRAM_ATTR handleInterrupt() { button = true; }
int buttonPin = 0;
...
void OnDataRecv(const uint8_t *mac,
  ...
  if (Recieve_Data.nBusStop == 0) {
    if (Recieve_Data.a == "BUS STOPPED!") {
      ...
      // flag = 1;
      delay(1000);
      send_data(broadcastAddress, (uint8_t *)&Send_Data,
                sizeof(Send_Data));
...
void setup() {
  ...
  // recv CB to get recv packer info
...
void loop() {
  if (button == true) {  // If the boot button is pressed
    ...
    esp_err_t result =
        esp_now_send(busMAC, (uint8_t *)&msg, sizeof(msg));
  ...
  if (!halt) {
    ...
    if (P.displayAnimate()) {
      ...
      sprintf(Buffer, "%01d",
              nShowTime);  // the number "1" is the number
                           // of digits that you want always
                           // shown on the screen for this
                           // font we can have 5
      P.displayText(Buffer, PA_CENTER, 0, 0, PA_PRINT,
                    PA_NO_EFFECT);
    }
    ...
    if (!service) {
      if (nService == 0) {
        ...
        if (P.displayAnimate()) {
          P.displayText("OOS", PA_CENTER, 0, 0, PA_PRINT,
                        PA_NO_EFFECT);
